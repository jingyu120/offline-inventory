export function sha256(ascii: string): string {
  ...
  const numBytes = asciiBytes[lengthProperty];
  for (i = 0; i < numBytes; i++) {
    words[i >> 2] |= asciiBytes[i] << (24 - (i % 4) * 8);
  }

  // Padding
  words[numBytes >> 2] |= 0x80 << (24 - (numBytes % 4) * 8);
  words[(((numBytes + 8) >> 6) + 1) * 16 - 1] = asciiLength;
  ...
  for (i = 0; i < 8; i++) {
    const h = hash[i];
    const wordHex =
      ((h >>> 4) & 0x0fffffff).toString(16) + (h & 0xf).toString(16);
    hex += wordHex.padStart(8, '0');
  }
  return hex;
}
