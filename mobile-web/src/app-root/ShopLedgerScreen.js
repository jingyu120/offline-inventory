...
import { Box, Text, Card, Button, Table } from '@burma-inventory/ui-components';
...
export function ShopLedgerScreen() {
  ...
  const loadShops = async () => {
    try {
      const shopsCollection = database.collections.get('shops');
      const regionsCollection = database.collections.get('regions');
  ...
  const handleSeedData = async () => {
    try {
      await database.write(async () => {
        const regionsCol = database.collections.get('regions');
        const r1 = await regionsCol.create((r) => {
          r.name = 'Yangon Division';
          r.division = 'Yangon';
        });
        const shopsCol = database.collections.get('shops');
        ...
        const contactsCol = database.collections.get('contacts');
        ...
        const logsCol = database.collections.get('interaction_logs');
...
//# sourceMappingURL=ShopLedgerScreen.js.map
